...
ModelMaterial::ModelMaterial()
	: number(-1)
	, ambientFile(""), emissiveFile(""), diffuseFile(""), specularFile(""), normalFile("")
	, ambientView(NULL), diffuseView(NULL), specularView(NULL), emissiveView(NULL), normalView(NULL)
{
...
void ModelMaterial::SetNormal(D3DXCOLOR & color)
{
	shaderBuffer->SetNormal(color);
}
...
void ModelMaterial::SetNormalTexture(string file)
{
	SAFE_RELEASE(normalView);
