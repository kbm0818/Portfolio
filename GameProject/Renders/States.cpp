...
ID3D11SamplerState* States::samplerDefault;
ID3D11DepthStencilState* States::depthDefault;
ID3D11DepthStencilState* States::depthOff;

ID3D11RasterizerState* States::rasterClock;
ID3D11RasterizerState* States::rasterCountClock;
ID3D11RasterizerState* States::rasterOffCullMode;
...
void States::Create()
{
	...
	GetSamplerDesc(&samDesc);
	CreateSampler(&samDesc, &samplerDefault);

	D3D11_DEPTH_STENCIL_DESC depthDesc;
	GetDepthStencilDesc(&depthDesc);
	CreateDepthStencil(&depthDesc, &depthDefault);

	depthDesc.DepthEnable = false;
	CreateDepthStencil(&depthDesc, &depthOff);

	D3D11_RASTERIZER_DESC rasterDesc;
	States::GetRasterizerDesc(&rasterDesc);
	rasterDesc.FrontCounterClockwise = false;
	States::CreateRasterizer(&rasterDesc, &rasterClock);

	States::GetRasterizerDesc(&rasterDesc);
	rasterDesc.FrontCounterClockwise = true;
	States::CreateRasterizer(&rasterDesc, &rasterCountClock);

	States::GetRasterizerDesc(&rasterDesc);
	rasterDesc.CullMode = D3D11_CULL_NONE;
	States::CreateRasterizer(&rasterDesc, &rasterOffCullMode);
}
...
void States::SetDepthOff()
{
	D3D::GetDC()->OMSetDepthStencilState(depthOff, 1);
}

void States::SetDepthDefault()
{
	D3D::GetDC()->OMSetDepthStencilState(depthDefault, 1);
}

void States::SetSamplerDefault(UINT slot, UINT count)
{
	D3D::GetDC()->PSSetSamplers(slot, count, &samplerDefault);
}

void States::SetRasterClock()
{
	D3D::GetDC()->RSSetState(rasterClock);
}
void States::SetRasterCountClock()
{
	D3D::GetDC()->RSSetState(rasterCountClock);
}

void States::SetRasterOffCullMode()
{
	D3D::GetDC()->RSSetState(rasterOffCullMode);
}
