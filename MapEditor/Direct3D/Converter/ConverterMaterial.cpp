...
ConverterMaterial::ConverterMaterial()
	: number(0)
...
ConverterMaterial::ConverterMaterial(int number, FbxSurfaceMaterial * material)
	: number(number)
	, ambient(0, 0, 0, 1), emissive(0, 0, 0, 1), diffuse(0, 0, 0, 1), specular(0, 0, 0, 1)
	, shininess(0.0f)
	, ambientFile(L""), emissiveFile(L""), diffuseFile(L""), specularFile(L"")
...
void ConverterMaterial::Export(BinaryWriter * bw)
{
	bw->Write(number);
	bw->Write(ambient);
	bw->Write(emissive);
...
void ConverterMaterial::Import(BinaryReader * br)
{
	number = br->UInt();
	ambient = br->Color4f();
	emissive = br->Color4f();
...
********************************************************************************/
D3DXCOLOR ConverterMaterial::GetProperty(FbxSurfaceMaterial * material, const char * name, const char * factorName, wstring* textureName)
{
...
void ConverterMaterial::GetProperty(FbxSurfaceMaterial * material, const char * name, wstring * textureName)
{
	FbxProperty prop = material->FindProperty(name);
...
********************************************************************************/
void ConverterMaterial::CreateView(wstring path, ID3D11ShaderResourceView ** view)
{
	...
	filePath = ConverterScene::TexturePath + filePath;
