...
void Texture::LoadTexture(string fileName, ID3D11Texture2D** texture)
{
	HRESULT hr;

	wstring temp = String::StringToWString(fileName);
...
bool Texture::LoadTexture(wstring fileName, ID3D11ShaderResourceView ** view)
{
	HRESULT result;


	// Load the texture in.
	result = D3DX11CreateShaderResourceViewFromFile(D3D::GetDevice(), fileName.c_str(), NULL, NULL, view, NULL);
	if (FAILED(result))
	{
		return false;
	}
